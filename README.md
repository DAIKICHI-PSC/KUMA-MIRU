...
［概要］  
本プログラムは、AIで「熊」を検出するプログラムです。  
最近の熊による被害に対して、何か貢献出来ないかと思い、作成しました。  

実行出来るプログラムは、三種類あります。  
kuma_miru　熊を検出する、基本となるプログラム（物体検出結果の変数external_outputを使用し、自由に処理を作成して下さい）。  
sample_kuma_miru_mail　熊を検出したら、メールを送信するプログラム。  
sample_kuma_miru_sound　熊を検出したら、犬の鳴き声を再生するプログラム。  

設定ファイル（settings.txt）内の物体番号を変更すれば、人間等も高精度に検出可能です（防犯用途等にも使用出来ます）。  
...
リモートデスクトップやVNCを使用すれば、リモートで管理出来ます。  
オープンソースのネットワークソフトウェア、SoftEther VPNを使用する事で、柔軟なネットワークを使用した運用管理が出来ます（ https://www.vpnazure.net/ja/ ）。  
ネットワークが無い環境でも、「AIR-Stick WiFi（株式会社ニッチカンパニー社）」等を使用すれば、安価に通信が出来ます。  
ソーラーバッテリーを使用すれば、電源が無い環境でも運用出来ます。  
パソコン起動後、検出プログラムも自動起動するように設定する事をお勧めします。  
  
用途に応じて、PCを選択して下さい（後付けGPUは高価なので、今回は非対応にしました）。  
...
各Pythonファイルか、各実行形式ファイルを実行して下さい。  
